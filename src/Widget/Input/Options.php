 ...
 * @method $this multiple(bool $value) 设置是否支持多选，默认为 false
 * @method $this labelField(string $value) 设置标识选项中哪个字段是label值，默认为 "label"
 * @method $this valueField(string $value) 设置标识选项中哪个字段是value值，默认为 "value"
 * @method $this deferField(string $value) 设置标识选项中哪个字段是defer值，默认为 "defer"
 * @method $this joinValues(bool $value) 设置是否拼接value值，默认为 true
 * @method $this extractValue(bool $value) 设置是否将value值抽取出来组成新的数组，只有在joinValues是false时生效，默认为 false
 * @method $this delimiter(string $value) 设置拼接符，默认为逗号
 * @method $this itemHeight(int $value) 设置每个选项的高度，用于虚拟渲染，默认为 32
 * @method $this virtualThreshold(int $value) 设置在选项数量超过多少时开启虚拟渲染，默认为 100
 * @method $this valuesNoWrap(bool $value) 设置默认情况下多选所有选项都会显示，通过这个可以最多显示一行，超出的部分变成 ...，默认为 false
 * @method $this searchable(bool $value) 设置是否可搜索，默认为 false
 * @method $this autoComplete(string|array $value) 设置自动补全配置
 * @method $this selectFirst(bool $value) 设置是否默认选择第一个选项，默认为 false
