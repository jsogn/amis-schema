 ...
 * @method $this level(string $value = 'default') 按钮样式，支持：link、primary、secondary、info、success、warning、danger、light、dark、default
 * @method $this size(string $value = '') 按钮大小，支持：xs、sm、md、lg
 ...
 * @method $this active(bool $value = false) 按钮是否高亮
 * @method $this activeLevel(string $value = '') 按钮高亮时的样式，配置支持同level
 * @method $this activeClassName(string $value = 'is-active') 给按钮高亮添加类名
 * @method $this block(bool $value = false) 用display:"block"来显示按钮
 * @method $this confirmText(string $value = '') 当设置后，操作在开始前会询问用户，可用 ${xxx} 取值
 * @method $this confirmTitle(string $value = '') 确认框标题，前提是 confirmText 有内容，支持模版语法
 * @method $this reload(string $value = '') 指定此次操作完后，需要刷新的目标组件名字（组件的name值），多个请用逗号隔开
 * @method $this tooltip(string|array $value = '') 鼠标停留时弹出该段文字，也可以配置对象类型，可用 ${xxx} 取值
 * @method $this disabledTip(string|array $value = '') 被禁用后鼠标停留时弹出该段文字，也可以配置对象类型，可用 ${xxx} 取值
 * @method $this tooltipPlacement(string $value = 'top') 如果配置了tooltip或者disabledTip，指定提示信息位置，可配置top、bottom、left、right
 * @method $this close(bool|string $value = false) 当action配置在dialog或drawer的actions中时，配置为true指定此次操作完后关闭当前dialog或drawer
 * @method $this required(array $value = []) 配置字符串数组，指定在form中进行操作之前，需要指定的字段名的表单项通过验证
 * @method $this api(string|array|API $value = '') ajax类型时的请求地址
 ...
 * @method $this blank(bool $value = false) 如果为true将在新tab页面打开
 ...
 * @method $this countDown(int $value = 0) 倒计时时间（秒）
 * @method $this countDownTpl(string $value = '') 倒计时显示文本模板
 * @method $this hotKey(string $value = '') 键盘快捷键配置
 * @method $this body(array|Widget|Widget[]|string $value = '') 作为容器组件时渲染的其他组件
 ...
 * @method $this disabled(bool $value = false) 按钮失效状态
 * @method $this loading(bool $value = false) 显示按钮 loading 效果
