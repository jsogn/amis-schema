 ...
 * @method self deferApi(API $value) 当行数据中有 defer 属性时，用此接口进一步加载内容
 * @method self affixHeader(bool $value = true) 是否固定表头
 * @method self affixFooter(bool $value = false) 是否固定表格底部工具栏
 * @method self columnsTogglable(string|bool $value = 'auto') 展示列显示开关, 自动即：列数量大于或等于 5 个时自动开启
 * @method self placeholder(string $value = '暂无数据') 当没数据的时候的文字提示
 * @method self className(string $value = 'panel-default') 外层 CSS 类名
 * @method self showIndex(bool $value) 是否展示序号
 * @method self tableClassName(string $value = 'table-db table-striped') 表格 CSS 类名
 * @method self headerClassName(string $value = 'Action.md-table-header') 顶部外层 CSS 类名
 ...
 * @method self resizable(bool $value = true) 列宽度是否支持调整
 * @method self selectable(bool $value = false) 支持勾选
 * @method self multiple(bool $value = false) 勾选 icon 是否为多选样式checkbox，默认为radio
 * @method self lazyRenderAfter(int $value = 100) 用来控制从第几行开始懒渲染行，用来渲染大表格时有用
 * @method self tableLayout(string $value = 'auto') 表格布局模式，当配置为 fixed 时，内容将不会撑开表格，自动换行
 ...
 * @method self reUseRow(bool|string|API $value) 默认当 API 返回数据与当前一致时不会触发表格行重渲染，为 false 总会触发重渲染，尽量复用返回数据中 id 一致的对象
 * @method self persistKey(string $value) 用于配置列排序、列显示的本地缓存所使用的 key
 * @method self prefixRowClassName(string $value) 顶部总结行 CSS 类名
 * @method self prefixRowClassNameExpr(string $value) 顶部总结行 CSS 类名表达式
 * @method self affixRowClassName(string $value) 底部总结行 CSS 类名
 * @method self affixRowClassNameExpr(string $value) 底部总结行 CSS 类名表达式
 */
